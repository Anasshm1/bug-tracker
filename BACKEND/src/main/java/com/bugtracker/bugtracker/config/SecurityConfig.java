...
public class SecurityConfig {
    ...
    // Comment Spring trouve un utilisateur par son email
    @Bean
    ...
    // Relie UserDetailsService + PasswordEncoder pour Spring
    @Bean
    public AuthenticationProvider authenticationProvider() {
        DaoAuthenticationProvider authProvider = new DaoAuthenticationProvider();
        authProvider.setUserDetailsService(userDetailsService());
        authProvider.setPasswordEncoder(passwordEncoder());
        log.info("AuthenticationProvider configuré avec BCrypt");
        return authProvider;
    }

    // Permet d'utiliser AuthenticationManager dans AuthService
    @Bean
    public AuthenticationManager authenticationManager(AuthenticationConfiguration config) throws Exception {
        return config.getAuthenticationManager();
    }

    // Configuration principale : routes, sessions, filtres
    @Bean
    ...
    // Autorise le frontend React (Vite port 5173) à parler avec l'API
    @Bean
